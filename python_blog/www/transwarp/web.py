...
WSGI概要：
    ...
    作用：将HTTP原始的请求、解析、响应这些交给WSGI server完成，
          让我们专心用Python编写Web业务，也就是 SGI处理函数
          所以WSGI 是HTTP的一种高级封装。
    例子：
        wsgi 处理函数
...
class Request(object):
    ...
    def _get_raw_input(self):
        '''
        将从wsgi解析出来的数据字典，添加为Request对象的属性,然后返回该字典
    ...
    def _get_cookies(self):
        """
        从environ里取出cookies字符串，并解析成键值对组成的字典
        """
...
class Response(object):
    ...
    def headers(self):
        '''
        setter构造的属性，以[(key1, value1), (key2, value2)...]形式存储所有header的值，包括cookies的值
    ...
    def header(self, name):
        '''
        获取Response Header里单个Header的值，大小写不敏感
    ...
    def content_type(self):
        '''
        setter方法实现的属性，用户保存header：Content-Type的值
    ...
    def content_type(self, value):
        '''
        让content_type属性可写，及设置Content-Type Header
