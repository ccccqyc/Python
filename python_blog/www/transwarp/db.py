...
def next_id(t=None):
    '''
    生成一个唯一50位id,由 当前时间 + 随机数（由伪随机数得来）拼接得到
    Return next id as 50-char string.
    # make a random UUID
    >>> uuid.uuid4().hex
    'd5675bda27424133bededa95e4869abb'
...
class _LasyConnection(object):
    ...
    def rollback(self):
        self.connection.rollback()# global engine object:
...
class _ConnectionCtx(object):
    '''
    因为_DbCtx实现了连接的获取和释放，但是并没有实现连接的自动获取和释放，
    _ConnectCtx在 _DbCtx基础上实现了该功能，因此可以对 _ConnectCtx 使用with 语法，
    比如：
...
def connection():
    '''
    db模块核心函数，用于获取一个数据库连接,通过_ConnectionCtx对 _db_ctx封装，
    使得惰性连接可以自动获取和释放，也就是可以使用 with语法来处理数据库连接
...
def update(sql, *args):
    ...
    >>> u2.email
    u'michael@test.org'
    >>> u2.passwd    执行update 语句，返回update的行数
    u'123456'
